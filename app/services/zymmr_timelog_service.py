...
convert_zymmr_report_to_timelog_entries → logout → TimelogService.save_report.

Login/session/logout machinery lives in zymmr_client.py, shared with
zymmr_effort_service.py (Effort Analyser's Zymmr sync).
"""
from datetime import date, datetime, timedelta

from .zymmr_client import (
    ZymmrAuthError, ZymmrRequestError, login_zymmr, logout_zymmr, post_report,
)
...
def _post_zymmr(session, sid, payload):
    """generate_report POST for the Timesheet Analyser's Zymmr report (QRY-96)."""
    base = current_app.config.get('ZYMMR_BASE_URL', 'https://flairminds.zymmr.com').rstrip('/')
    return post_report(session, sid, payload, f'{base}/frontend/analytics/reports/QRY-96')
